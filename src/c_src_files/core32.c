...
#include "../c_header_files/core32.h"
...
int encrypt_and_binaryify_32(const char *input, const char *output, const char *password)
{
    ...
    derive_key_32(password, salt, key);
    ...
    if (!g_silent)
    {
        printf("Encrypting (32 bit core) with AES-256-GCM + Argon2id...\n");
    ...
    write_bits(out, size_bytes, 4);
    write_bits(out, salt, SALT_SIZE);
    write_bits(out, iv, IV_SIZE);
    write_bits(out, tag, AES_TAG_SIZE);
...
int decrypt_and_run_32(const char *input, const char *password)
{
    ...
    unsigned char size_bytes[4];
    read_bits(in, size_bytes, 4);
    long original_size = (size_bytes[0] << 24) | (size_bytes[1] << 16) | (size_bytes[2] << 8) | size_bytes[3];
    (void)original_size;
    unsigned char salt[SALT_SIZE];
    unsigned char iv[IV_SIZE];
    unsigned char tag[AES_TAG_SIZE];
    read_bits(in, salt, SALT_SIZE);
    read_bits(in, iv, IV_SIZE);
    read_bits(in, tag, AES_TAG_SIZE);
    fseek(in, 0, SEEK_END);
