...
int install_context_menu()
{
    ...
    // Try HKEY_CLASSES_ROOT first (requires Admin)
    HKEY root = HKEY_CLASSES_ROOT;
    char cmd[512];
    snprintf(cmd, 512, "\"%s\" \"%%1\"", exe_path);

    // Attempt to write to HKCR
    int success = 1;
    if (!install_registry_key(root, ".bxe", NULL, "bxe_file")) success = 0;
    if (!install_registry_key(root, "bxe_file", NULL, "Binf Executable")) success = 0;
    if (!install_registry_key(root, "bxe_file\\DefaultIcon", NULL, exe_path)) success = 0; // rough icon
    if (!install_registry_key(root, "bxe_file\\shell\\open\\command", NULL, cmd)) success = 0;
    ...
    if (!success) {
        ...
        if (!install_registry_key(root, "Software\\Classes\\.bxe", NULL, "bxe_file")) success = 0;
        if (!install_registry_key(root, "Software\\Classes\\bxe_file", NULL, "Binf Executable")) success = 0;
        if (!install_registry_key(root, "Software\\Classes\\bxe_file\\DefaultIcon", NULL, exe_path)) success = 0;
        if (!install_registry_key(root, "Software\\Classes\\bxe_file\\shell\\open\\command", NULL, cmd)) success = 0;
    }
...
int add_to_path() {
    ...
    if (RegQueryValueExA(hKey, "Path", NULL, NULL, NULL, &size) != ERROR_SUCCESS) {
        // If empty or missing, create usually?
        size = 1; // dummy
    }
